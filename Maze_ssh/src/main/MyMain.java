...
public class MyMain {
    ...
    public static void main(String[] args) {
        ...
        login.dispose();
        File file1 = new File("src/data/archive/" + Session.session.get("username") + "_keep1.txt");
        File file2 = new File("src/data/archive/" + Session.session.get("username") + "_keep2.txt");
        File file3 = new File("src/data/archive/" + Session.session.get("username") + "_keep3.txt");
        try {
            if (!file1.exists())
                file1.createNewFile();
            if (!file2.exists())
